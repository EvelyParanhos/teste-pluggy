...
class SyncServiceTest {
    ...
    void shouldNotReconcileInvoiceStatusIfCreditTransactionIsNotACardPayment() {
        ...
        Invoice invoiceEntity = Invoice.builder()
                .id(300L)
                .pluggyBillId("bill-past")
                .account(accountEntity)
                .closeDate(LocalDate.of(2026, 8, 1))
        ...
        List<Invoice> savedInvoices = invoiceCaptor.getAllValues();
        Invoice finalInvoice = savedInvoices.get(savedInvoices.size() - 1);
        assertThat(finalInvoice.getStatus()).isNotEqualTo("PAID");
        assertThat(finalInvoice.getStatus()).isEqualTo("OVERDUE");
    }
}
